...
const mathCategories = {
  arithmetic: {
    label: "Arithmetic",
    topics: ["Fractions", "Decimals", "Percentages", "Ratios"]
  },
  preAlgebra: {
    label: "Pre-Algebra",
    topics: ["Order of Operations", "Factors & Multiples", "Exponents"]
  },
  algebra: {
    label: "Algebra",
    topics: ["Linear Equations", "Factoring Expressions", "Quadratic Equations"]
  }
};
...
export default function App() {
  ...
  const startQuiz = () => {
    alert(`Generating a ${difficulty} ${questionStyle} quiz for ${category} -> ${subTopic}!`);
    // We will wire this up to our question generation algorithms next
  };

  if (screen === 'setup') {
    return (
      <div style={{ padding: '40px', maxWidth: '450px', margin: '40px auto', border: '1px solid #ddd', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', background: 'white' }}>
        <h2 style={{ textAlign: 'center', marginTop: 0 }}>Quiz Setup Menu 🧮</h2>

        <div style={{ margin: '20px 0' }}>
          {/* Main Category Dropdown */}
          <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '6px' }}>Category:</label>
          <select
            value={category}
          ...
          >
            {Object.keys(mathCategories).map((key) => (
              <option key={key} value={key}>{mathCategories[key].label}</option>
            ))}
          </select>

          {/* Dynamic Sub-Topic Dropdown */}
          <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '6px' }}>Sub-Topic:</label>
          <select
            value={subTopic}
          ...
          {/* Difficulty Dropdown */}
          <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '6px' }}>Difficulty:</label>
          <select
            value={difficulty}
          ...
          <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '8px' }}>Question Style:</label>
        ...
        <button
          onClick={startQuiz}
          style={{ width: '100%', padding: '14px', background: '#0070f3', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold' }}
        >
          Generate Quiz
        </button>
      </div>
